function [p_now,k_p]=goto_goal_3d_5seg_for_video(times,p_now,k_p)
    k_dpddis = k_p(1);
    k_dpdph = k_p(2) ;
    k_dpdtheta= k_p(3);  
    k_dpdx = k_p(4) ;
...
% goal =[ -87.1349  -100.1586  400.8258    0.0205   -0.0166]
        x_dest = goal(1);
        y_dest = goal(2);
        z_dest = goal(3);
...
%         k_dpdx = ( x_change) / (4*(p_change(2)+p_change(4)-p_change(1)-p_change(3))+ 5*(p_change(6)+p_change(8)-p_change(5)-p_change(7)) + 7*(p_change(10)+p_change(12)-p_change(9)-p_change(11)) + 0.1 ); %Add 0.1 to avoid unavailable dividend
        ...
        if scale>0.5&&scale<2
            ...
            k_dpdtheta=max( k_dpdtheta, 0.0003 );
        else 
