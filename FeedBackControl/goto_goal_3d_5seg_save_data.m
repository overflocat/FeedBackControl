function [p_now,k_p]=goto_goal_3d_5seg_save_data(goal,time,p_now,k_p)
...
%     [k_dpddis,k_dpdtheta,k_dpdx] = SetkValues_XZ ( p_now )
%     [k_dpddis,k_dpdph,k_dpdy] = SetkValues_YZ ( p_now )
...
%zhu yi IP
        ...
        X_error(i) = x_dest-stateone(1)  ;
        Y_error(i) =  y_dest-stateone(2) ;
        Z_error(i)= z_dest-stateone(3);
        ph_error(i) = ph_dest-stateone(4)  ;
        theta_error(i)=theta_dest-stateone(5);
...
%     state_list=[state_list;goal];
%      clock_time=fix(clock);
%     time = [date,'_',num2str(clock_time(4), '%02d'),'_',num2str(clock_time(5), '%02d'),'_',num2str(clock_time(6), '%02d')];
%     name = [time,'-',num2str(goal(1)),'-',num2str(goal(2)),'-',num2str(goal(3))]
%     save(name,'state_list');

end


%     load('9_1_18_Network1.mat');
%     net2=init_net(3,100);
%     clock_time=fix(clock);
%     name = [date,'_',num2str(clock_time(4)),'_',num2str(clock_time(5))];
%     filename_np_nxy=[name,'_NP_NXY.mat'];
% 
